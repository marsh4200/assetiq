...
- **Machines** — service schedule pulled from your asset register. Tap **Import** to bring in any asset filed under a machine group (a group whose name contains "machine", e.g. the default *Workshop Machines* / `WM`); trucks are left out so you can add them by hand. Each machine tracks by **months**, **kilometres** or **running hours** — whichever you select. The next service (date or meter reading) auto-rolls forward each time a service is logged, a full history is kept, and reminders fire ahead of time (a month before for time-based; within ~10% of the interval for km/hours) feeding the Overview dashboard + bell. Use **Update km/hrs** on a card to keep its current reading fresh between services.
